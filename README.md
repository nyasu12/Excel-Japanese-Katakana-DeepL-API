...
公開リポジトリへAPIキーを誤ってコミットしないよう、**Windowsの環境変数 `DEEPL_AUTH_KEY` を使う方法を推奨**します。

PowerShellの例:

```powershell
[Environment]::SetEnvironmentVariable("DEEPL_AUTH_KEY", "YOUR_DEEPL_API_KEY", "User")
```

設定後はExcelをいったん終了し、再起動してください。VBAは `DEEPL_AUTH_KEY` を自動で読み取ります。

どうしても環境変数を使えない場合のみ、`src/modDeepL.bas` の次のフォールバック欄をローカルで設定できます。

```vb
Private Const DEEPL_AUTH_KEY_FALLBACK As String = ""
```

**実際のAPIキーを設定した状態の `.bas` ファイルはGitHubへコミットしないでください。**
...
- DeepL APIキーは環境変数 `DEEPL_AUTH_KEY` から読む構成を推奨します。
- リポジトリ内のコード内フォールバックは空欄のままにしてください。
- `.env`、`api_key.txt`、`secrets.txt`、Excelブックなどは `.gitignore` で除外しています。
